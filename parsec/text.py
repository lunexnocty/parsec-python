...
from parsec import IStream, IState, Parser, Context, token, tokens, item, sel, seq
...
decinteger = (
    seq(num_sign, digit.some().map("".join))
    .map(lambda x: int("".join(x), base=10))
    .expected("decimal integer")
)
bininteger = (
    seq(num_sign, bindigit.many().prefix(seq(char("0"), item.range("bB"))).map("".join))
    .map(lambda x: int("".join(x), base=2))
    .expected("binary integer")
)
octinteger = (
    seq(num_sign, octdigit.many().prefix(seq(char("0"), item.range("oO"))).map("".join))
    .map(lambda x: int("".join(x), base=8))
    .expected("octal integer")
)
hexinteger = (
    seq(num_sign, hexdigit.many().prefix(seq(char("0"), item.range("xX"))).map("".join))
    .map(lambda x: int("".join(x), base=16))
...
_pointfloat = sel(
    seq(_digitpart.default(""), _fraction).map("".join),
...
number = floatnumber.or_else(integer).expected("number")
blanks = blank.many().map("".join).expected("blanks")
identifier = (
    seq(sel(alpha, underline), sel(alnum, underline).many().map("".join))
    .map("".join)
    .expected("identifier")
)
