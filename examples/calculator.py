    ...
    def test_valid_expressions(self):
        ...
        for text, expected in test_cases:
            with self.subTest(msg=f"Testing: {text}"):
                ...
                self.assertEqual(result.outcome.value.exec(), expected, msg=f"Failed for: {text}")
    ...
    def test_numerical_precision(self):
        ...
        for text, expected in test_cases:
            with self.subTest(msg=f'Testing: {text}'):
                result = parse(expr, text)
                self.assertIsInstance(result.outcome, Okay)
                self.assertAlmostEqual(result.outcome.value.exec(), expected, delta=1e-12, msg=f"Failed for: {text}")
