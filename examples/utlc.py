...
class TestLambdaParser(unittest.TestCase):
    ...
    def test_nested_structures(self):
        test_cases = [
            ...
            (
                r'\a->a b c',
                Abs(Var('a'), App(App(Var('a'), Var('b')), Var('c'))),
            ),  # left_associative_app
        ]
        for text, expected in test_cases:
            with self.subTest(msg=f'Testing: {text}'):
                result = parse(lam_expr, text)
    ...
    def test_edge_cases(self):
        test_cases = [
            (
                r'\x y z->x',
                Abs(Var('x'), Abs(Var('y'), Abs(Var('z'), Var('x')))),
            ),  # multi_param_arrow
        ...
        for text in test_cases:
            with self.subTest(msg=f'Testing: {text}'):
                ...
                self.assertIsInstance(
                    result.outcome, Fail, msg=f'Should fail but passed: {text}'
                )
    ...
    def test_advanced_syntax(self):
        test_cases = [
            ...
            (
                r'\x->x \y->y',
                Abs(Var('x'), App(Var('x'), Abs(Var('y'), Var('y')))),
            ),  # abs_in_app
            (r'x \y->y', App(Var('x'), Abs(Var('y'), Var('y')))),  # app_with_abs
            (
                r'\x->(\y->y) x',
                Abs(Var('x'), App(Abs(Var('y'), Var('y')), Var('x'))),
            ),  # nested_abs_in_app
        ]
        for text, expected in test_cases:
            with self.subTest(msg=f'Testing: {text}'):
    ...
    def test_complex_expressions(self):
        test_cases = [
            (
                r'(\x->x) (\y->y) z',
                App(App(Abs(Var('x'), Var('x')), Abs(Var('y'), Var('y'))), Var('z')),
            ),  # nested_apps
            (
                r'\x->\y->\z->x y z',
                Abs(
                    Var('x'),
                    Abs(
                        Var('y'), Abs(Var('z'), App(App(Var('x'), Var('y')), Var('z')))
                    ),
                ),
            ),  # deep_nested_abs
            (
                r'(\x->x y) (\z->z)',
                App(Abs(Var('x'), App(Var('x'), Var('y'))), Abs(Var('z'), Var('z'))),
            ),  # complex_app
        ]
        for text, expected in test_cases:
            with self.subTest(msg=f'Testing: {text}'):
